...
def evaluate(model, dataset = "cifar10", method = "normal"):
    ...
    if(dataset == "cifar10"):
        ...
        if(method == "pencil"):
            ...
            test_loader = torch.utils.data.DataLoader(pencil_test,batch_size=batch_size,shuffle=False)   
    ...
    if(dataset == "cifar100"):
        ...
        if(method == "pencil"):
            ...
            test_loader = torch.utils.data.DataLoader(pencil_test,batch_size=batch_size,shuffle=False)   
    if(dataset == "cub200"):
        test_loader=torch.utils.data.DataLoader(cub200_test,batch_size=batch_size,shuffle=False)
        if(method == "pencil"):
            pencil_test = Dataset_with_Indices(cub200_test)
            test_loader = torch.utils.data.DataLoader(pencil_test,batch_size=batch_size,shuffle=False)   
    ...
    if(method == "pencil"):
        with torch.no_grad():
           ...
           for j,(images,labels, indices) in enumerate(test_loader):
               images = images.to(device)
               labels = labels.to(device)
...
def fgsm_evaluate( model, dataset = "cifar10", method = "normal", epsilon = 0.1 ): #This code is from https://pytorch.org/tutorials/beginner/fgsm_tutorial.html
    ...
    if(dataset == "cifar100"):
        ...
        if(method == "pencil"):
            ...
            test_loader = torch.utils.data.DataLoader(pencil_test,batch_size=1,shuffle=False)   
    if(dataset == "cub200"):
        test_loader=torch.utils.data.DataLoader(cub200_test,batch_size=1,shuffle=False)
        normalize = cub200_normalize
        mean = cub200_mean
        std = cub200_std
        if(method == "pencil"):
            pencil_test = Dataset_with_Indices(cub200_test)
    ...
    if (method == "pencil"):
        for i,(image, label, indice) in enumerate(test_loader):
    
            
            image, label, indice = image.to(device), label.to(device), indice.to(device)
            
           
            image.requires_grad = True
    
            prediction, learned_label, log_label = model(image, indice)
            _,predicted = torch.max(prediction,1)
...
img_txt = 'data/CUB_200_2011/test_list.txt'
cub200_test = Cub200Dataset(img_path, img_txt, cub200_transform)
