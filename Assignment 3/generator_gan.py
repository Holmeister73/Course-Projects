# -*- coding: utf-8 -*-
"""
Created on Sun May 14 23:48:35 2023
...
def generate(network_name):
    ...
    with torch.no_grad():
        ...
        gen = pickle.load(open(network_name, "rb")).to(device)
        gen.eval()
        generated = gen(random_noise_plot)
        
        plt.figure(figsize = (60, 40))
        plt.gray()
        imgs = generated.cpu().detach().numpy()
        for i, item in enumerate(imgs):
            if i >= 101: break
            plt.subplot(10, 10, i+1)
            plt.imshow(item[0])

# generate("model_gan.pk")  generates 100 images from random noise via weights in model_gan.pk
...
# FID_Score(1000, "model_gan.pk") generates 1000 fake images and also uses 1000 real images from training set to calculate the FID score
# KID_Score(1000, 1000, 100, "model_gan.pk") generates 1000 fake images and uses 100 subsets of size 500 to calculate KID score.
# Note: FID score gets lower as sample size gets larger but KID score doesn't have that kind of bias. 
# However using larger sample sizes makes computation more difficult in terms of memory and time consumption. I suggest 1000.
# I used 10000 sample size in the report since running once was enough for the report.

#print("\nKID Score:", KID_Score(1000, 500, 100, "model_gan.pk"))
#print("FID Score:", FID_Score(1000, "model_gan.pk"))
#generate("model_gan.pk")

#generate("model_wasserstein_gan.pk")
#print("\nKID Score:", KID_Score(1000, 500, 100, "model_wasserstein_gan.pk"))
#print("FID Score:", FID_Score(1000, "model_wasserstein_gan.pk"))
